...
def extract_menus(
    soup, allergy_map: Dict[str, str], restaurant_name: str
) -> List[Dict[str, Any]]:
    menus = []
    ...
    for idx, td in enumerate(tds):
        ...
        for item in menu_items:
            if "vegetable salad & dressing" in item.lower():
                ...
                for a in allergies:
                    ...
                    if re.match(r"^[\d,\s]+$", a):
                        allergy_numbers.extend(
                            [x.strip() for x in a.split(",") if x.strip()]
                        )
                ...
                for num in allergy_numbers:
                    if num in allergy_map:
                        ingredients.append(
                            {"name": allergy_map[num], "description": ""}
                        )
                price = None
                price_pattern = re.compile(
                    r"(?:([\d,]+)[\s\-]*(₩|KRW|Won|won)|(?:₩|KRW|Won|won)[\s\-]*([\d,]+))",
                    re.IGNORECASE,
                )
        ...
        for item in new_menu_items:
            ...
            if salad_ingredients:
                ...
                allergy_numbers = list(
                    set(allergy_numbers) | set(salad_allergy_numbers or [])
                )
            menus.append(
                {
                    "meal": meal,
...
def postprocess_cafeteria_menus(
    menus: List[Dict[str, Any]], allergy_map: Dict[str, str]
) -> List[Dict[str, Any]]:
    processed = []
    for menu in menus:
        if "<Cafeteria>" in menu["menu_name"]:
            ...
            for part in parts:
                ...
                price = (
                    float(
                        (price_match.group(1) or price_match.group(3)).replace(",", "")
                    )
                    if price_match and (price_match.group(1) or price_match.group(3))
                    else None
                )
                ...
                menu_name_clean = re.sub(
                    r"(₩|KRW|Won|won)?\s*[\d,]+(₩|KRW|Won|won)?",
                    "",
                    menu_name_clean,
                    flags=re.IGNORECASE,
                ).strip()
                ...
                if price is not None:
                    ...
                    if prev_menu:
                        ...
                        existing_names = set(
                            i["name"] for i in prev_menu["main_ingredients"]
                        )
