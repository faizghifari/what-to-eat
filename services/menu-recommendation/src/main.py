...
def delete_restaurant(restaurant_id: str):
    ...
    if restaurant[0]["image_url"] is not None:
        image_path = restaurant[0]["image_url"].split("/")[-1]
...
def list_matches_restaurant(
    ...
    user_profile = (
        supabase.table("Profile").select("*").eq("id", user.id).execute()
    ).data[0]
    ...
    for menu in menus:
        ...
        matches_restaurant_ids.add(menu["restaurant"])

    restaurants = (
        supabase.table("Restaurant")
        .select("*")
        .in_("id", list(matches_restaurant_ids))
        .execute()
    ).data
...
def list_restaurant_menus(x_user_uuid: Annotated[str, Header()], restaurant_id: str):
    ...
    restaurant_location = Location(
        **(
            supabase.table("Location")
            .select("*")
            .eq("id", restaurant.data[0]["location"])
            .execute()
        ).data[0]
    )

    return RestaurantMenuResponse(
        restaurant=Restaurant(**restaurant.data[0]),
